...
##
## A [UtilityAICurve] describes a relationship between a normalized
...
@export_range(-100, 100) var exponent: int = 1:
	set(value):
		exponent = value
		_update_points()

## The slope of the curve (if applicable).
@export_range(-100, 100) var slope: int = 1:
	set(value):
		slope = value
		_update_points()
...
func _to_string() -> String:
	...
	var fmt := "[<UtilityAICurve#%d>: %s, (%d, %d, %f, %f)]"
	return (
