...
var utility = {"move_to_enemy": 0, "attack_enemy": 0} # score-based behavior
var is_cutscene := false # If we are in a cutscene, we want fixed behavior
...
func update_animation():
	...
	elif Input.is_action_just_pressed("attack_button"):
		attack_dir = last_direction
	
	animation_tree.set("parameters/conditions/swing", 
		Input.is_action_just_pressed("attack_button")
		|| Input.is_action_just_pressed("attack_mouse"))
